...
// 2. Top 100 por País
router.get('/top-country/:country', async (req, res) => {
  const session = driver.session();
  try {
...
router.get('/api/recommendations/for-user/:userId', async (req, res) => {
  const session = driver.session();
  try {
...
router.get('/api/recommendations/by-genres', async (req, res) => {
  const session = driver.session();
  try {
...
router.get('/api/recommendations/similar-artists/:artistId', async (req, res) => {
  const session = driver.session();
  try {
...
router.get('/api/recommendations/collaborative/:userId', async (req, res) => {
  const session = driver.session();
  try {
    const { userId } = req.params;
...
router.get('/api/recommendations/discover-local/:userId', async (req, res) => {
  const session = driver.session();
  try {
    const { userId } = req.params;
