...
const QueuePanel = ({ isOpen, onClose }) => {
  ...
  const getSongTitle = (song) => {
    return song?.titulo || song?.title || t('song.untitled');
  };
    ...
    <div className="queue-panel-overlay" onClick={onClose}>
      <div className="queue-panel" onClick={(e) => e.stopPropagation()}>
        <div className="queue-panel-header">
          <h3 className="queue-title">{t('player.queue')}</h3>
          <div className="queue-header-actions">
            ...
            <button onClick={onClose} className="close-queue-btn" aria-label={t('player.close')}>
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="queue-panel-content">
          {currentSong && (
            <div className="queue-section">
              <h4 className="queue-section-title">{t('player.nowPlaying')}</h4>
          ...
          {queue.length > 0 ? (
            <div className="queue-section">
              <h4 className="queue-section-title">
                {t('player.upNext', {
                  n: queue.length,
                  songs: queue.length === 1 ? t('player.song') : t('player.songs'),
                })}
              </h4>
                    ...
                    >
                      ...
                      >
                        ...
                        <button
                          ...
                          aria-label={t('player.playSong', { title: getSongTitle(song) })}
                        >
                          <Play size={16} />
                        </button>
                      </div>
                      ...
                      <button
                        ...
                        aria-label={t('player.removeFromQueue', { title: getSongTitle(song) })}
                      >
                        <X size={16} />
                      </button>
                    </div>
            ...
            !currentSong && (
              <div className="queue-empty">
                ...
                <p className="queue-empty-title">{t('player.queueEmpty')}</p>
                <p className="queue-empty-subtitle">
                  {t('player.queueEmptySub')}
                </p>
              </div>
            )
          )}
          ...
          {history.length > 0 && (
            <div className="queue-section">
              <h4 className="queue-section-title">{t('player.history')}</h4>
