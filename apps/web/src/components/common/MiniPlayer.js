...
const MiniPlayer = () => {
  ...
  const songTitle = currentSong.titulo || currentSong.title || t('song.untitled');
  ...
  const albumCover = currentSong.album_info?.portada_url 
    ? `/api/music/covers/${currentSong._id}.png` 
      ...
      <div className="mini-player-content">
        ...
        <div className="mini-player-extra-controls">
          <button
            ...
            title={t('player.queueBadge', { n: queue.length })}
          >
            <ListMusic size={20} />
            {queue.length > 0 && (
          ...
          <button
            ...
            aria-label={isExpanded ? t('player.minimize') : t('player.expand')}
