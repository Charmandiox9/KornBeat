...
const PlaylistContent = () => {
  ...
  const deletePlaylist = async (playlistId) => {
    if (!window.confirm(t('pl.confirmDelete'))) return;
                ...
                <div className="playlist-header">
                  ...
                  <div className="playlist-header-info">
                    ...
                    <div className="playlist-stats">
                      <span>{t('pl.songs', { n: selectedPlaylist.total_canciones })}</span>
                      <span>•</span>
                      <span>{formatTotalDuration(selectedPlaylist.duracion_total)}</span>
                      <span>•</span>
                      <span>{selectedPlaylist.reproducciones} {t('song.playCount')}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="playlist-songs">
                  {playlistSongs.length === 0 ? (
                    <div className="empty-playlist-songs">
                      <Music size={48} />
                      <p>{t('pl.emptyPlaylist')}</p>
                      <p className="subtitle">{t('pl.emptyPlaylistSub')}</p>
                    </div>
                  ) : (
                    <div className="songs-table">
                      <div className="songs-table-header">
                        <div className="col-number">#</div>
                        <div className="col-title">{t('pl.colTitle')}</div>
                        <div className="col-album">{t('pl.colAlbum')}</div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <h2>{t('pl.createTitle')}</h2>
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <label>{t('pl.titleLabel')}</label>
            <input
              type="text"
              value={formData.titulo}
              onChange={(e) => setFormData({...formData, titulo: e.target.value})}
              placeholder={t('pl.titlePh')}
              required
              autoFocus
            />
          </div>
          ...
          <div className="form-group">
            <label>{t('pl.descLabel')}</label>
            <textarea
              value={formData.descripcion}
              onChange={(e) => setFormData({...formData, descripcion: e.target.value})}
              placeholder={t('pl.descPh')}
              rows={3}
            />
          </div>
